...
static void rotate_right(char *vector[], int size) {
  ...
  /* size 分ローテート */
  while (size--) vector[size] = vector[size - 1];
...
/* Functions */
int dis_getopt(GCC_UNUSED int argc, char *argv[], const char *options) {
  int index;       /* 現在のインデックス */
  int next;        /* 次のインデックス */
  int optchar;     /* オプション文字 */
...
again:
  ...
  /* すでに終りか？ */
  if (string == 0) {
    if (savepoint > 0) optind = savepoint;
    init = 1;
    return EOF;
  }
  ...
  if (*string == '-') {
    ...
    /* 正確に "-" か？ならば普通の引数 */
    if ((optchar = *string++) == '\0') goto normal_arg;
    ...
    for (ptr = options; *ptr; ptr++)
      if (*ptr != ':' && *ptr == optchar) break;

    /* 引数を伴う場合ならばインデックスは初期化 */
    if (*string == '\0' || ptr[1] == ':') comidx = 1;
  ...
  normal_arg:
    ...
    /* 引数の位置を記憶し、次のオプションを調べる */
      ...
      if (savepoint == 0) savepoint = index;
      index++;
      goto again;
    }
  }
